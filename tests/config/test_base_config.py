...
class TestBaseConfig(unittest.TestCase):
    ...
    def test_config_types(self):
        """Testing basic config types"""
        with self.assertRaises(TypeError):
            cfg = ConfigTypes(
                INT_ARG=10, FLOAT_ARG=math.pi, BOOL_ARG=False, STR_ARG='abc',
            ...
            self.assertIsInstance(cfg.DATETIME_ARG, datetime)
            self.assertIsInstance(cfg.DATE_ARG, date)
            self.assertIsInstance(cfg.TIMEDELTA_ARG, timedelta)
            self.assertEqual(math.pi, cfg.FLOAT_ARG)
    ...
    def test_compose_config(self):
        ...
        cfg = Config(
            ...
            SUB_CONFIGS=[{'ARG_1': 2, 'ARG_2': 'EFGH'},
                         {'ARG_1': 3, 'ARG_2': 'IJKL'}])
    ...
    def test_load_from_file(self):
        ...
        with tempfile.NamedTemporaryFile('w', delete=True) as tmp:
            tmp.write(
                '{"TEST_ALPHA": "ALPHA", '
                '"TESTING_BETA": "BETA", "TEST_GAMMA": true}')
            tmp.flush()
